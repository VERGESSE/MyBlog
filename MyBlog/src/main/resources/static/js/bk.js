// "use strict";
{
    ...
    // let titleH = document.getElementById("page-title").offsetHeight
    // let allH = document.getElementById("blog").offsetHeight
    const run = () => {
        ...
        // let newHeight = window.pageYOffset - titleH > 0 ? window.pageYOffset - titleH : 0
        // if (newHeight >= allH - window.innerHeight*2)
        //     newHeight = allH - window.innerHeight*2
        // document.getElementById("canvas").style.marginTop=newHeight+'px';
        if (frame > 60){
            ctx.drawImage(webgl.elem, 0, 0);
        }
    };
    requestAnimationFrame(run);
